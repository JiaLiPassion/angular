...
readonly projectRoot=$(cd ${thisDir}/../..; pwd)
setEnvVar PROJECT_ROOT ${projectRoot}
...
if [[ ${TRAVIS:-} ]]; then
  case ${CI_MODE} in
    js)
      ...
      ;;
    saucelabs_required)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('${projectRoot}/browser-providers.conf').sauceAliases.CI_REQUIRED.join(','))"`
      ;;
    browserstack_required)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('${projectRoot}/browser-providers.conf').browserstackAliases.CI_REQUIRED.join(','))"`
      ;;
    saucelabs_optional)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('${projectRoot}/browser-providers.conf').sauceAliases.CI_OPTIONAL.join(','))"`
      ;;
    browserstack_optional)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('${projectRoot}/browser-providers.conf').browserstackAliases.CI_OPTIONAL.join(','))"`
      ;;
  esac
else
...
if [[ ${TRAVIS:-} ]]; then
  ...
  readonly SAUCEKEY=${SAUCE_ACCESS_KEY:-}
  export NGBUILDS_IO_KEY=${NGBUILDS_IO_KEY:-$SAUCEKEY}
